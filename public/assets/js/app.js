  ...
  $('.saved-buttons').on('click',  function () {
      ...
      $.get("/comments/" + thisId, function(data){
          ...
          for(var i = 0; i<data.comment.length; i++) {
              var button = ' <a href=/deleteComment/' + data.comment[i]._id + '><i class="pull-right fa fa-times fa-2x deletex" aria-hidden="true"></i></a>';
              $('#commentsBody').append('<div class="panel panel-default"><div class="commentText panel-body">' + data.comment[i].body + '  ' + button + '</div></div>');
          }
      });
  });
